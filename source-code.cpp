//code starts here
#include<graphics.h> // to include the graphics file
#include<iostream>
#include<cstdio>
using namespace std;
struct coin // structure for the coloured coin(goti)
{
 int pos;
 int status;
 char cllr;
...
struct user // structure to store user data
{
 struct coin g[4];
 int def;
 int color;
...
void play(void);
void star(int x,int y) // commands to create the star image in graphics
{
 line(x-7,y-7,x,y+10);
 line(x,y+10,x+7,y-7);
...
void flood_fill(int x,int y,int newColor,int oldColor) // commands to fill colour in visulas
{
int c;
c=getpixel(x,y);
if(c==oldColor)
...
void cir(int x,int y,int c) // commands to create the circle image in graphics
{
 int r=13;
 circle(x+28,y+28,r);
...
void layout(void) // commands to create the LUDO board by calling all the other functions like star,cirlce, line, rectangle, in graphics
{
 int i=0,x,y,j;
 setcolor(12);
...
void play() // commands to start the play and take input
{
 struct user *student;
 int n,k=1;
 int num;
 ...
 num = (rand() % 6 ) +1;
 printf("\tDICE : %d :",num);
 printf("\n\tTO MOVE Select the coin : (0,1,2,3) ::");
 scanf("%d",&run); // which coin to move
 ...
 if(getcl != 0 && getcl != color_g[i] && first+num != 1 && first+num != 9 &&
first+num != 14 && first+num != 22 && first+num != 27 && first+num != 35 && first+num != 40 &&
first+num !=48 )
